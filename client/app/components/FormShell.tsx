...
export function FormFieldHeader({ label, required = false, detail }: FormFieldHeaderProps) {
  return (
    <div className="mb-2 flex items-center justify-between gap-3">
      ...
      {detail ? <span className="app-shell-mono text-[10px] uppercase tracking-[0.2em] text-[var(--app-outline)]">{detail}</span> : null}
    </div>
  );
}
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-6">
      ...
      <header className="space-y-2">
        {eyebrow ? <p className="app-shell-mono text-xs uppercase tracking-[0.28em] text-[var(--app-outline)]">{eyebrow}</p> : null}
        <div>
      ...
      <section className="rounded-[1.75rem] bg-[var(--app-surface-container-low)] p-6 outline outline-1 outline-[var(--app-outline-variant-soft)] sm:p-8">
        {error ? (
          <div className="mb-6 rounded-2xl bg-[var(--app-error-container)]/20 px-4 py-3 text-sm text-[var(--app-error)] outline outline-1 outline-[var(--app-error)]/10">
            {error}
          </div>
        ) : null}
