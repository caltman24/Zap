...
export function getDeadlineIcon(dueDate: string, className: string = ""): JSX.Element {
    ...
    switch (status) {
        ...
        case 'due-soon':
            return <span
                className={`material-symbols-outlined text-[var(--app-tertiary)] ${className}`}>schedule</span>;
        default:
...
export function getDeadlineStatusBadge(dueDate: string, variant: 'compact' | 'detailed' = 'detailed'): JSX.Element | null {
    ...
    switch (status) {
        case 'overdue':
            ...
            return (
                <span
                    className={`${deadlineBadgeBaseClass} bg-[var(--app-error-container)]/20 text-[var(--app-error)] outline outline-1 outline-[var(--app-error)]/10`}>
          {overdueText}
        </span>
            );
        case 'due-soon':
            ...
            return (
                <span
                    className={`${deadlineBadgeBaseClass} bg-[var(--app-tertiary-container)]/20 text-[var(--app-tertiary)] outline outline-1 outline-[var(--app-tertiary)]/10`}>
          {dueSoonText}
        </span>
            );
        default:
