import {type ActionFunctionArgs, type HeadersFunction, type LoaderFunctionArgs, redirect,} from "@remix-run/node";
                ...
                {isEditing ? (
                        ...
                        <Form className="space-y-8" encType="multipart/form-data" method="post">
                            ...
                            <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
                                <div className="flex w-full max-w-xs self-start flex-col gap-4 lg:w-72">
                                    <div
                                        className="overflow-hidden rounded-2xl bg-[var(--app-surface-container-low)] outline outline-1 outline-[var(--app-outline-variant-soft)]">
                                        <div className="aspect-square w-full bg-[var(--app-surface-container-lowest)]">
                                            {!removeLogo ? (
                                                ...
                                                <div
                                                    className="grid h-full place-items-center text-[var(--app-outline)]">
                                                    <span
                                                        className="material-symbols-outlined text-6xl">image_not_supported</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="min-w-0 flex-1 space-y-6">
                                    <div>
                                        <label className="mb-2 block text-sm font-medium text-[var(--app-on-surface)]">Company
                                            Name</label>
                                        <input
                                    ...
                                    <div>
                                        <label
                                            className="mb-2 block text-sm font-medium text-[var(--app-on-surface)]">Description</label>
                                        <textarea
                    ...
                    <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                        <div className="flex min-w-0 flex-1 flex-col gap-5 sm:flex-row sm:items-start">
                            <div
                                className="w-fit shrink-0 self-start overflow-hidden rounded-2xl bg-[var(--app-surface-container-lowest)] p-1.5 outline outline-1 outline-[var(--app-outline-variant)]/10">
                                <div
                                    className="h-20 w-20 overflow-hidden rounded-[1.05rem] bg-[var(--app-surface-container-low)] sm:h-24 sm:w-24">
                                    <img alt={`${companyInfo.name} logo`} className="h-full w-full object-cover"
                                         src={companyInfo.logoUrl ?? companyPlaceholderLogo}/>
                                </div>
                            </div>
            ...
            <section className="space-y-6">
                ...
                <div className="grid gap-x-10 gap-y-8 lg:grid-cols-2">
                    {memberGroups.map((group) => (
                        <section className="space-y-3 border-t border-[var(--app-outline-variant)]/10 pt-4"
                                 key={group.role}>
                            <div className="flex items-center justify-between gap-3">
                                <h3 className="text-base font-semibold text-[var(--app-on-surface)]">{group.label}</h3>
                                <span
                                    className="app-shell-mono text-[10px] uppercase tracking-[0.22em] text-[var(--app-outline)]">
