...
export default function SideMenu({menuRoutes, company, onNavigate, onClose}: SideMenuProps) {
    ...
    return (
        <aside
            className="flex h-full w-full flex-col bg-[var(--app-surface)] py-5 text-[var(--app-on-surface)] lg:w-[15.5rem] lg:py-6">
            <div className="mb-7 px-5 lg:px-7">
                ...
                {company ? (
                    <div
                        className="mt-5 flex items-center gap-3 rounded-2xl bg-[var(--app-surface-container-low)] px-2 py-2 outline outline-1 outline-[var(--app-outline-variant)]/10">
            <span
                className="inline-flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[var(--app-surface-container-high)]">
              <img
                        ...
                        return (
                            <div className="flex flex-col gap-2.5" key={group[0]?.to ?? index}>
                                ...
                                {link.materialIcon}
                                                        ...
                                                        <span
                                                            className={`${railLabelClass} ${active ? "text-[var(--app-primary-fixed-strong)]" : ""}`}>{link.name}</span>
                                                    </>
                                                );
                                            }}
                                        </NavLink>
            ...
            <div className="mt-auto flex flex-col gap-3 px-3 pb-1 lg:px-4">
                ...
                <div className="flex flex-col gap-2.5">
                    <Form method="post">
                        ...
                        >
                            <span
                                className={`${railIconBaseClass} group-hover:text-[var(--app-on-surface)]`}>logout</span>
