import {Form, Link, type UIMatch, useFetcher, useMatches} from "@remix-run/react";
            ...
            <div className="flex min-h-14 items-start justify-between gap-4 px-4 py-2.5 sm:px-6 lg:px-8">
                <div className="flex min-w-0 flex-1 items-start gap-4 lg:gap-6">
                    ...
                    <div className="min-w-0 flex-1 space-y-2">
                        <div className="relative w-full max-w-md" ref={searchRootRef}>
                            <div
                                className="group relative w-full min-w-0 rounded-lg bg-[var(--app-surface-container-lowest)] shadow-[var(--app-search-shadow)] transition-[background-color,box-shadow] duration-200 focus-within:shadow-[var(--app-search-focus-shadow)]">
                <span
                    className="material-symbols-outlined pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-[color:var(--app-outline)]">
                  search
                </span>
                                <input
                                ...
                                {searchFetcher.state !== "idle" ? (
                                    <span
                                        className="absolute right-3 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-[var(--app-primary)] animate-pulse"/>
                                ) : null}
                            </div>
                            ...
                            {shouldShowSearchDropdown ? (
                                ...
                                >
                                    ...
                                    {!searchError && searchFetcher.state !== "idle" && searchResults.length === 0 ? (
                                        <div
                                            className="px-4 py-3 text-sm text-[var(--app-on-surface-variant)]">Searching...</div>
                                    ) : null}

                                    {!searchError && searchFetcher.state === "idle" && searchResults.length === 0 ? (
                                        <div className="px-4 py-3 text-sm text-[var(--app-on-surface-variant)]">No
                                            tickets or projects found.</div>
                                    ) : null}
                                            ...
                                            {searchResults.map((result) => (
                                                ...
                                                >
                                                    <div className="min-w-0">
                                                        <div
                                                            className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-[var(--app-outline)]">
                                                            <span
                                                                className={monoClass}>{getSearchResultTypeLabel(result)}</span>
                                                            {result.displayId ? <span
                                                                className={monoClass}>{result.displayId}</span> : null}
                                                        </div>
                                                        <div
                                                            className="mt-1 truncate text-sm font-medium text-[var(--app-on-surface)]">{result.name}</div>
                                                    </div>
                                                    <span
                                                        className="material-symbols-outlined mt-0.5 text-base text-[var(--app-outline)]">arrow_outward</span>
                                                </Link>
                                            ))}
                                        </div>
                                    ) : null}
                        ...
                        <nav aria-label="Breadcrumb" className="hidden min-w-0 items-center gap-2 text-xs lg:flex">
                            {breadcrumbMatches.map((match, index) => (
                                <div className="flex min-w-0 mt-2 items-center gap-2"
                                     key={`${match.id}-${getBreadcrumbLabel(match)}`}>
                                    {index > 0 ? (
                                        <span
                                            className={`${monoClass} text-[10px] text-[color:var(--app-outline)]`}>/</span>
                                    ) : null}
                                    <span
                                        className="min-w-0 truncate text-[color:var(--app-on-surface-variant)] [&_a]:truncate [&_a]:text-inherit [&_a]:transition-colors [&_a]:hover:text-[var(--app-on-surface)]">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <DropdownMenu
                        ...
                        trigger={
                            <span
                                className="inline-flex h-8 w-8 items-center justify-center overflow-hidden rounded-full border border-[var(--app-avatar-border)] transition-transform hover:-translate-y-0.5">
                {avatarUrl ? (
