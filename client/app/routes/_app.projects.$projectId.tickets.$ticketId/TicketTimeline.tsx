...
function getHistoryIconMeta(type: number) {
    switch (type) {
        ...
        case HistoryTypes.Unarchived:
            return {
                icon: "inventory_2",
                className: "bg-[var(--app-surface-container-high)] text-[var(--app-on-surface-variant)]"
            };
    ...
    if (loading) {
        return (
            <div className="grid min-h-[220px] place-items-center py-8">
                <span
                    className="inline-flex h-5 w-5 animate-spin rounded-full border-2 border-[var(--app-outline)] border-r-transparent"/>
            </div>
        );
    }
                ...
                {history.map((entry, index) => {
                    ...
                    return (
                        <div className="relative flex gap-4" key={entry.id}>
                            {showConnector ? (
                                <div
                                    className="absolute bottom-0 left-5 top-12 w-px bg-[color:var(--app-outline-variant)]/12"/>
                            ) : null}

                            <div
                                className={`z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${iconMeta.className}`}>
                                <span className="material-symbols-outlined text-lg">{iconMeta.icon}</span>
                            </div>

                            <div
                                className="min-w-0 flex-1 space-y-2 rounded-2xl bg-[var(--app-surface-container-lowest)]/70 px-4 py-4 outline outline-1 outline-[var(--app-outline-variant)]/10">
                                <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--app-outline)]">
                                    <span
                                        className="app-shell-mono uppercase tracking-[0.2em]">{formatDate(entry.createdAt)}</span>
                                    <span className="h-1 w-1 rounded-full bg-[var(--app-outline)]/50"/>
                                    <span>{entry.creator.name}</span>
                                    <span
                                        className="rounded-full bg-[var(--app-surface-container-high)] px-2 py-0.5 text-[10px] text-[var(--app-on-surface-variant)]">
                    {entry.creator.role}
                  </span>
                                </div>
