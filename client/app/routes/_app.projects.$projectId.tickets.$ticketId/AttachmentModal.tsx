...
export default function AttachmentModal({attachment, isOpen, onClose, onDownload}: AttachmentModalProps) {
    ...
    function renderPreview() {
        ...
        if (attachment.type === "application/pdf") {
            return (
                <div
                    className="rounded-2xl bg-[var(--app-surface-container-lowest)] px-6 py-10 text-center outline outline-1 outline-[var(--app-outline-variant)]/10">
        ...
        if (attachment.type === "text/plain") {
            return (
                <div
                    className="rounded-2xl bg-[var(--app-surface-container-lowest)] p-4 outline outline-1 outline-[var(--app-outline-variant)]/10">
                    {isLoading ? (
                        <div className="grid min-h-[220px] place-items-center">
                            <span
                                className="inline-flex h-6 w-6 animate-spin rounded-full border-2 border-[var(--app-outline)] border-r-transparent"/>
                        </div>
                    ) : (
                        <pre
                            className="max-h-[60vh] overflow-auto whitespace-pre-wrap text-sm leading-6 text-[var(--app-on-surface)]">{textContent}</pre>
                    )}
                </div>
            );
        }
        ...
        return (
            <div
                ...
                <p className="mt-2 text-sm text-[var(--app-on-surface-variant)]">This file type cannot be previewed in
                    the browser.</p>
            </div>
        );
    }
    ...
    return (
        <dialog
            className="m-auto w-full max-w-4xl overflow-visible border-0 bg-transparent px-4 py-0 text-left text-[var(--app-on-surface)] shadow-none backdrop:bg-black/70 backdrop:backdrop-blur-sm sm:px-6 lg:px-0"
            onClick={(event) => {
        ...
        >
            <div
                ...
                <div
                    className="flex items-start justify-between gap-4 border-b border-[var(--app-outline-variant)]/10 px-6 py-5 sm:px-8">
                    <div className="min-w-0 flex-1">
                        <h3 className="truncate text-xl font-bold text-[var(--app-on-surface)]">{attachment.name}</h3>
                        <div
                            className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[var(--app-on-surface-variant)]">
                            <span>{formatFileSize(attachment.size)}</span>
                            <span className="h-1 w-1 rounded-full bg-[var(--app-outline)]/50"/>
                            <div className="flex items-center gap-2">
                                <img alt={attachment.uploadedBy.name}
