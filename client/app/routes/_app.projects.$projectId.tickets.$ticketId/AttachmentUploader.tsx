...
export default function AttachmentUploader({ onFileUpload, maxFileSize, maxTotalSize, currentTotalSize }: AttachmentUploaderProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <div className="space-y-4">
          <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--app-surface-container-high)] text-[var(--app-outline)] transition-all duration-200 ${isDragOver ? "scale-105 text-[var(--app-primary)]" : ""}`}>
            <span className="material-symbols-outlined text-4xl">upload_file</span>
          </div>

          <div>
            <p className="text-lg font-semibold text-[var(--app-on-surface)]">Drop files here or click to browse</p>
            ...
            <p className="mt-1 text-xs text-[var(--app-outline)]">
              Max file size: {formatFileSize(maxFileSize)} · Remaining space: {formatFileSize(remainingSize)}
            </p>
          </div>
        </div>
      </button>
      ...
      {Object.keys(uploadProgress).length > 0 ? (
        <div className="space-y-3 rounded-2xl bg-[var(--app-surface-container-lowest)]/80 px-4 py-4 outline outline-1 outline-[var(--app-outline-variant)]/10">
          <h4 className="text-sm font-semibold text-[var(--app-on-surface)]">Uploading</h4>
          {Object.entries(uploadProgress).map(([fileId, progress]) => (
            <div className="space-y-2" key={fileId}>
              <div className="flex items-center justify-between gap-3 text-xs text-[var(--app-on-surface-variant)]">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-[var(--app-surface-container-high)]">
                <div className="h-full rounded-full bg-[linear-gradient(90deg,var(--app-primary),var(--app-primary-fixed))] transition-all duration-200" style={{ width: `${progress}%` }} />
              </div>
            </div>
          ))}
        </div>
      ...
      {errors.length > 0 ? (
        <div className="rounded-2xl bg-[var(--app-error-container)]/20 px-4 py-4 text-sm text-[var(--app-error)] outline outline-1 outline-[var(--app-error)]/10">
