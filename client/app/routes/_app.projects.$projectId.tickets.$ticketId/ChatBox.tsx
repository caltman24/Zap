...
export default function ChatBox({ className, comments, userId, loading, onDeleteComment, onEditComment }: ChatBoxProps) {
  ...
  if (loading) {
    return (
      <div className={`${className ?? ""} grid min-h-[220px] place-items-center`.trim()} ref={scrollableChatContainerRef}>
        <span className="inline-flex h-7 w-7 animate-spin rounded-full border-2 border-[var(--app-outline)] border-r-transparent" />
      </div>
    );
  }
  ...
  if (!comments || comments.length === 0) {
    return (
      <div className={`${className ?? ""} flex min-h-[220px] flex-col items-center justify-center text-center`.trim()} ref={scrollableChatContainerRef}>
        ...
        <p className="mt-1 text-sm text-[var(--app-on-surface-variant)]">Start the conversation by adding a comment below.</p>
      </div>
    );
  }
            ...
            <div key={comment.id}>
              {showDateHeader ? (
                <div className="flex justify-center py-1">
                  <span className="app-shell-mono rounded-full bg-[var(--app-surface-container-high)] px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-[var(--app-outline)]">
              ...
              <div className={`group flex gap-3 ${isOwnComment ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`max-w-[82%] space-y-2 ${isOwnComment ? "items-end text-right" : "items-start text-left"}`}>
                  <div className={`flex items-center gap-2 text-xs ${isOwnComment ? "justify-end" : "justify-start"}`}>
                    <span className="font-medium text-[var(--app-on-surface)]">{comment.sender.name}</span>
                    <time className="app-shell-mono text-[var(--app-outline)]">{formatCommentTime(comment.createdAt)}</time>
                  </div>
                  ...
                  <div className={`flex gap-2 ${isOwnComment ? "justify-end" : "justify-start"}`}>
                    {!isOwnComment && (comment.capabilities.canEdit || comment.capabilities.canDelete) ? (
                      <div className="flex flex-col gap-1 pt-2 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                  ...
                  {comment.updatedAt ? (
                    <div className={`app-shell-mono text-[10px] text-[var(--app-outline)] ${isOwnComment ? "text-right" : "text-left"}`}>
